...
export function ProductManager({ session, onRefresh }: ProductManagerProps) {
  ...
  const [targetMenu, setTargetMenu] = useState<string>('all');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tablesToInsert: string[] = [];
      if (targetMenu === 'all' || targetMenu === 'student') tablesToInsert.push('student_menu');
      if (targetMenu === 'all' || targetMenu === 'professional') tablesToInsert.push('proff_menu');
      if (targetMenu === 'all' || targetMenu === 'elite') tablesToInsert.push('elite_menu');
      ...
      alert('Failed to save product to one or more segmented menu tables');
    } finally {
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-mono text-gray-500 uppercase tracking-widest pl-2">Target Menu Segment</label>
                  <select
                    value={targetMenu}
                    onChange={e => setTargetMenu(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white focus:outline-none focus:border-[#D4FF00]/50"
                  >
                    <option value="all">⚡ All 3 Menus (Global Sync)</option>
                    <option value="student">🎓 Student Menu (student_menu)</option>
                    <option value="professional">💼 Professional Menu (proff_menu)</option>
                    <option value="elite">🏆 Elite Athlete Menu (elite_menu)</option>
                  </select>
                </div>
              </div>
