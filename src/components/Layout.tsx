...
function Header({ session }: { session: Session | null }) {
  ...
  const navItems = [
    ...
    // If we've visited the fuel station, hide the Home nav item completely
  ...
  return (
    <>
      ...
      <div className="md:hidden sticky top-0 z-40 border-b border-white/5 bg-[#050505]/80 backdrop-blur-md px-4 py-4 flex justify-between items-center">
        <Link to={hasVisitedFuelStation ? "/menu" : "/"} className="flex items-center gap-2 group">
        ...
        <footer className="border-t border-white/5 bg-[#000000] py-16 mt-20 hidden md:block">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <Link to={hasVisitedFuelStation ? "/menu" : "/"} className="flex items-center gap-3">
                <Dumbbell className="h-8 w-8 text-[#D4FF00]" />
                <span className="font-display text-2xl tracking-wide uppercase font-black">
                  Wheyo
                </span>
              </Link>
