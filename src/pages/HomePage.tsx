...
import { ArrowRight, Zap, Dumbbell, Target, GraduationCap, Briefcase, Trophy, ChevronRight, User, Mail, Sparkles, Loader2, Lock } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import { motion, useScroll, useTransform, AnimatePresence } from 'motion/react';
import { AnimatedCounter } from '../components/AnimatedCounter';
import { supabase, getPublicUrl } from '../lib/supabase';
import { setHasVisitedFuelStation } from '../components/Layout';
...
export default function HomePage({ session }: { session: Session | null }) {
  ...
  // Netflix styled Identity selector overlay
  const [showIdentitySelector, setShowIdentitySelector] = useState(false);

  const handleIntroComplete = async () => {
    setHasVisitedFuelStation(true);
    localStorage.setItem('has_completed_intro', 'true');
    setShowIntro(false);

    if (session?.user?.id) {
      // If user is logged in, try to fetch previously selected segment from database first
      try {
  ...
  useEffect(() => {
    const fetchRandomProducts = async () => {
      if (!supabase) return;
      const segment = localStorage.getItem('user_segment') || 'student';
      const tableName = segment === 'professional' ? 'proff_menu' : segment === 'elite' ? 'elite_menu' : 'student_menu';
        ...
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-30 text-center px-4 max-w-5xl mx-auto pt-32 pb-48 md:pt-40 md:pb-64">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2 }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
              <Link 
                to="/menu" 
                onClick={() => setHasVisitedFuelStation(true)}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-[#D4FF00] text-black px-8 py-4 md:px-10 md:py-5 rounded-full font-bold text-base md:text-xl hover:bg-white transition-all hover:scale-105 shadow-[0_0_30px_rgba(212,255,0,0.3)]"
              >
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
            {(precisionProducts.length > 0 ? precisionProducts.slice(0, 4) : [
              ...
              return (
                <motion.div key={product.id} onClick={() => { setHasVisitedFuelStation(true); navigate('/menu'); }} style={{ y: isMobile ? 0 : cardYs[index] }} className={`group relative h-[350px] md:h-[450px] rounded-3xl overflow-hidden border border-white/10 cursor-pointer ${index % 2 !== 0 ? 'md:mt-24' : ''}`}>
