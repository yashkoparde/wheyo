...
import { CheckCircle2, ShoppingBag, ArrowRight, MessageSquare, MapPin, Calendar, Flame } from 'lucide-react';
...
export default function OrderSuccessPage() {
  ...
  const { orderId, customerName, pickupPoint, items, finalPrice, orderProtein } = orderDetails;
      ...
      <div className="max-w-xl mx-auto text-center">
        ...
        >
          ...
          <div className="space-y-6">
            ...
            {orderProtein > 0 && (
              <div className="mt-4 p-4 bg-[#D4FF00]/5 border border-[#D4FF00]/20 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Flame className="w-5 h-5 text-[#D4FF00]" />
                  <span className="text-xs font-mono font-bold uppercase tracking-wider text-gray-300">Gains Secured</span>
                </div>
                <span className="text-xl font-display text-[#D4FF00]">+{orderProtein}g Pro</span>
              </div>
            )}
          </div>
        </motion.div>
